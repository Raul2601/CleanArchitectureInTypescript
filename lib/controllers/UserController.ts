...
import { insufficientParameters, mongoError, successResponse, failureResponse } from '../app/persistance/common/service';
...
export class UserController implements IBaseController<UserBusiness>{
    ...
    public create(req: Request, res: Response) {
        ...
        if (req.body.name && req.body.name.first_name && req.body.name.middle_name && req.body.name.last_name &&
            ...
            let user_params: IUser = <IUser>req.body;
            user_params.modification_notes = [{
                modified_on: new Date(Date.now()),
                modified_by: null,
                modification_note: 'New user created'
            }];

            this.UserBusiness.create(user_params, (err, user) => {
                if (err) {
                    mongoError(err, res);
                } else {
